...
### 이진 탐색 트리 응용 : 영어 사전
-----
1. 이진 탐색 트리를 이용해 영어 사전 구현
2. 프로그램 메뉴
```
**** i : 입력, d : 삭제, s : 탐색, p : 출력, q : 종료 ****
```

3. 가장 중요한 메뉴는 입력, 탐색, 삭제
   - 입력이 선택되면, 사용자로부터 단어와 그 의미를 받아 insert_node 함수를 이용해 이진 탐색 트리에 저장
   - 탐색이 선택되면, 사용자로부터 단어를 입력받아 이진 탐색 트리에서 search 함수를 이용해 탐색한 다음, 그 의미를 화면에 출력
   - 삭제가 선택되면, delete_node 함수를 이용해 단어를 찾아 삭제

4. 이진 탐색 트리에 저장되는 데이터
   - element 타입 구조체로 정의하고, 구조체 안 필요한 필드를 넣어야 함
   - 영어 단어와 그 의미가 각 저장되어야 하므로 두 개의 문자열 배열 필요
   - word 배열과 meaning 필드를 가지도록 정의
   - word 필드는 탐색의 기준이 되는 키 필드
```c
...
// 데이터 형식
typedef struct {
    char word[MAX_WORD_SIZE]; // key 필드
...
5. 두 개의 element 항목의 순서를 비교하는 compare 함수
   - compare(e1, e2) : 알파벳 순서로 e1과 e2를 비교하여 e1이 e2보다 작으면 -1, 같으면 0, 크면 1을 반환
   - strcmp 함수를 이용해 구현
```c
// 두 개의 단어 비교 함수
/*
...
void display(TreeNode* p) {
    if(p != NULL) { // 트리가 모두 순회할 때까지
        ...
        printf("%s : %s", p->key.word, p->key.meaning); // 루트 노드 출력
